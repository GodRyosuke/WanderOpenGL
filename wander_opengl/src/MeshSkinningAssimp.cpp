...
{
	MeshAssimp::ReserveVertexSpace();
	m_Bones.resize(mNumVertices);
}

void MeshSkinningAssimp::LoadMesh(const aiMesh* pMesh, unsigned int meshIdx)
{
	MeshAssimp::LoadMesh(pMesh, meshIdx);
...
void MeshSkinningAssimp::PopulateBuffers()
{
	GLuint m_boneBuffer;
	// 頂点データを読み込む
	MeshAssimp::PopulateBuffers();
...
void MeshSkinningAssimp::CalcInterpolatedRotation(aiQuaternion& Out, float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
...
void MeshSkinningAssimp::CalcInterpolatedScaling(aiVector3D& Out, float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
    // we need at least two values to interpolate...
...
void MeshSkinningAssimp::CalcInterpolatedPosition(aiVector3D& Out, float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
    // we need at least two values to interpolate...
...
void MeshSkinningAssimp::ReadNodeHierarchy(float AnimationTimeTicks, const aiNode* pNode, const glm::mat4& ParentTransform)
{
    GLUtil glutil;
...
void MeshSkinningAssimp::GetBoneTransform(float TimeInSeconds, std::vector<glm::mat4>& Transforms)
{
